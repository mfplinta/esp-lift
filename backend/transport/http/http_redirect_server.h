#ifndef HTTP_REDIRECT_SERVER_H
#define HTTP_REDIRECT_SERVER_H
...
static esp_err_t http_redirect_server_handler(httpd_req_t *req) {
  http_redirect_server_config_t *config = (http_redirect_server_config_t *) req->user_ctx;
...
static esp_err_t http_redirect_server_register(httpd_handle_t server,
                                               const http_redirect_server_config_t *config) {
  if (!server) return ESP_ERR_INVALID_ARG;

  http_redirect_server_config_t *cfg = malloc(sizeof(http_redirect_server_config_t));
  ...
  *redirect_get = (httpd_uri_t) {.uri = path,
                                 .method = HTTP_GET,
                                 .handler = http_redirect_server_handler,
                                 .user_ctx = cfg};
...
static esp_err_t http_redirect_server_start(httpd_handle_t *server_out,
                                            const http_redirect_server_config_t *config) {
  if (!server_out) return ESP_ERR_INVALID_ARG;

  httpd_config_t server_config = HTTPD_DEFAULT_CONFIG();
  server_config.uri_match_fn = httpd_uri_match_wildcard;
  server_config.lru_purge_enable = config ? config->lru_purge_enable : true;
  server_config.keep_alive_enable = config ? config->keep_alive_enable : true;
  server_config.max_uri_handlers = config && config->max_uri_handlers ? config->max_uri_handlers : 4;
  server_config.server_port = config && config->server_port ? config->server_port : 80;

  esp_err_t err = httpd_start(server_out, &server_config);
  if (err != ESP_OK) return err;

  err = http_redirect_server_register(*server_out, config);
  if (err != ESP_OK) {
